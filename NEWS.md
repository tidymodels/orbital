...
* `mlp()` with the `"keras3"` engine is now supported for regression and classification, reusing the same per-neuron column generation as bare keras3 `Sequential` support (#149).
